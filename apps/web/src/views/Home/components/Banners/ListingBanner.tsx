...
const PerpetualBanner = () => {
  ...
  const perpetualUrl = useMemo(
    () => `https://docs.shadowswap.xyz/${perpLangMap(code)}/the-team/sverse-lab?theme=${perpTheme(isDark)}`,
    [code, isDark],
  )
  ...
  return (
    <S.Wrapper>
      <S.Inner>
        <S.LeftWrapper>
          <S.StyledSubheading ref={headerRef}>{t('Tokens Listing')}</S.StyledSubheading>
